import { useEffect, useState } from "react";
import { Link } from "wouter";
import { apiFetch } from "@/lib/auth";
import { Briefcase, BarChart3, BookOpen, Bell, FileText, Heart, Mic, MessageSquare, User } from "lucide-react";

export default function CandidatoDashboard() {
  const [stats, setStats] = useState<any>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    apiFetch("/api/candidate/stats").then(setStats).catch(() => {}).finally(() => setLoading(false));
  }, []);

  const cards = [
    { label: "VAGAS ENCONTRADAS", value: stats?.totalJobsFound ?? "—", icon: Briefcase, href: "/candidato/vagas", color: "#FACC15" },
    { label: "MATCH SCORE MÉDIO", value: stats?.avgMatchScore ? `${stats.avgMatchScore}%` : "—", icon: BarChart3, href: "/candidato/analise", color: "#F97316" },
    { label: "CURSOS RECOMENDADOS", value: stats?.coursesRecommended ?? "—", icon: BookOpen, href: "/candidato/cursos", color: "#FACC15" },
    { label: "NOTificações", value: "—", icon: Bell, href: "/candidato/notificacoes", color: "#F97316" },
  ];

  const quickActions = [
    { label: "MEU PERFIL", icon: User, href: "/candidato/perfil" },
    { label: "UPLOAD CV", icon: FileText, href: "/candidato/cv" },
    { label: "SWIPE VAGAS", icon: Heart, href: "/candidato/swipe" },
    { label: "ENTREVISTAS", icon: Mic, href: "/candidato/entrevistas" },
    { label: "CARTA DE MOTIVAÇÃO", icon: MessageSquare, href: "/candidato/carta-motivacao" },
  ];

  return (
    <div className="min-h-screen bg-[#0A0A0A] p-6 md:p-12">
      <div className="max-w-[1400px] mx-auto">
        <h1 className="font-display text-6xl md:text-8xl text-white uppercase mb-12">
          Meu <span className="text-[#FACC15]">Painel</span>
        </h1>

        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {cards.map((card) => {
            const Icon = card.icon;
            return (
              <Link key={card.label} href={card.href}
                className="block bg-white/5 border-2 border-white/10 p-8 hover:border-[#FACC15] transition-colors group shadow-[8px_8px_0px_0px_rgba(249,115,22,0.3)]">
                <Icon size={32} className="text-[#FACC15] mb-4" />
                <p className="font-display text-5xl text-white group-hover:text-[#FACC15] transition-colors">{card.value}</p>
                <p className="font-bold text-sm text-white/50 uppercase tracking-widest mt-2">{card.label}</p>
              </Link>
            );
          })}
        </div>

        <h2 className="font-display text-4xl text-white uppercase mb-8">Acesso Rápido</h2>
        <div className="grid md:grid-cols-3 lg:grid-cols-5 gap-4">
          {quickActions.map((action) => {
            const Icon = action.icon;
            return (
              <Link key={action.href} href={action.href}
                className="flex flex-col items-center gap-4 bg-[#FACC15] text-[#0A0A0A] p-8 hover:bg-[#F97316] hover:text-white transition-colors shadow-[8px_8px_0px_0px_rgba(249,115,22,1)]">
                <Icon size={36} />
                <span className="font-display text-xl uppercase">{action.label}</span>
              </Link>
            );
          })}
        </div>
      </div>
    </div>
  );
}
