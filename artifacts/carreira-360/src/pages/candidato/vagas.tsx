  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] p-6 md:p-12">
      <div className="max-w-[1400px] mx-auto">
        ...
        <form onSubmit={handleSearch} className="bg-white/5 border-2 border-white/10 p-8 mb-12 shadow-[8px_8px_0px_0px_rgba(249,115,22,0.3)]">
          <div className="grid md:grid-cols-3 gap-6">
            <div className="md:col-span-2">
              ...
              <input value={keywords} onChange={e => setKeywords(e.target.value)} placeholder="Ex: JavaScript, React, Developer"
                className="w-full bg-white/5 border-2 border-white/20 text-white h-16 px-6 text-lg focus:border-[#FACC15] focus:outline-none rounded-none placeholder:text-white/30" />
            </div>
            <div>
          ...
          <button type="submit" disabled={searching}
            className="mt-6 h-16 bg-[#FACC15] hover:bg-[#F97316] text-[#0A0A0A] text-xl font-display uppercase px-12 transition-colors disabled:opacity-50 shadow-[8px_8px_0px_0px_rgba(249,115,22,1)]">
        ...
        {loading ? (
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {jobs.map((job: any) => (
              <div key={job.id} className="bg-white/5 border-2 border-white/10 p-8 hover:border-[#FACC15] transition-colors group">
