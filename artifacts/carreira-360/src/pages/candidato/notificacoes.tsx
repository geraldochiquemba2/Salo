      ...
      <div className="max-w-[800px] mx-auto">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-12">
          <h1 className="font-display text-4xl sm:text-6xl md:text-8xl text-white uppercase">
            Notificações {unread > 0 && <span className="text-[#247BA0]">({unread})</span>}
          </h1>
          {unread > 0 && (
            <button onClick={markAllRead} className="h-12 bg-white/10 hover:bg-[#1B98E0] hover:text-[#13293D] text-white px-6 font-bold uppercase text-sm transition-colors flex items-center gap-2">
          ...
          <div className="space-y-4">
            {notifications.map((n: any) => (
              <div key={n.id} className={`border-2 p-6 flex items-start gap-4 transition-colors ${n.read ? "bg-white/5 border-white/10" : "bg-[#1B98E0]/5 border-[#1B98E0]"}`}>
                ...
                {!n.read && (
                  <button onClick={() => markRead(n.id)} className="text-[#1B98E0] hover:text-green-500 shrink-0"><Check size={20} /></button>
                )}
              </div>
            ))}
          </div>
