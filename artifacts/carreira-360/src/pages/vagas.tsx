    ...
    <div className="min-h-screen bg-[#13293D] text-white">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#13293D]/90 backdrop-blur-md border-b border-white/5">
        <div className="max-w-[1400px] mx-auto px-6 h-20 flex items-center justify-between">
          <Link href="/" className="font-display text-3xl text-[#1B98E0] hover:text-[#247BA0] transition-colors font-black italic tracking-tighter">TALENTOS</Link>
          <div className="hidden md:flex items-center gap-8">
            <Link href="/vagas" className="text-[#1B98E0] font-bold text-sm uppercase tracking-wider border-b-2 border-[#1B98E0] pb-1">Vagas</Link>
            <Link href="/" className="text-white hover:text-[#1B98E0] font-bold text-sm uppercase tracking-wider transition-colors">Empresas</Link>
            <Link href="/" className="text-white hover:text-[#1B98E0] font-bold text-sm uppercase tracking-wider transition-colors">Como Funciona</Link>
      ...
      <main className="pt-20">
        ...
        <div className="relative w-full h-[400px] md:h-[500px] overflow-hidden">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 md:p-12 max-w-[1400px] mx-auto">
            <div className="inline-flex items-center gap-2 bg-[#1B98E0]/10 border border-[#1B98E0]/30 text-[#1B98E0] px-4 py-2 font-bold text-sm uppercase tracking-wider mb-4">
            ...
            <h1 className="font-display text-[2rem] sm:text-[3rem] md:text-[6rem] uppercase leading-[0.85] mb-4">
              As Melhores<br /><span className="text-[#1B98E0]">Vagas</span> de Angola.
            </h1>
          </div>
        </div>
        ...
        <div className="max-w-[1400px] mx-auto px-6 pb-20">
          ...
          <div className="relative mb-8">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Cargo, empresa ou competência…"
              className="w-full bg-white/5 border-2 border-white/10 text-white h-18 pl-16 pr-6 text-lg focus:border-[#1B98E0] focus:outline-none rounded-none placeholder:text-white/30" />
          </div>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {categories.map(c => (
                <button key={c} onClick={() => setCategory(c)}
                  className={`px-5 py-2.5 font-bold text-sm uppercase tracking-wider transition-colors ${category === c ? "bg-[#1B98E0] text-[#13293D]" : "bg-white/5 text-white/60 hover:text-white border border-white/10"}`}>
                  {c}
                </button>
              ))}
            </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-24">
            {filtered.map(job => (
              <div key={job.id} className="bg-white/5 border-2 border-white/10 p-6 hover:border-[#1B98E0] transition-colors group relative flex flex-col">
                ...
                <h3 className="font-display text-2xl uppercase mb-2 group-hover:text-[#1B98E0] transition-colors">{job.title}</h3>
                ...
                <div className="mt-auto flex items-center justify-between">
                  <p className="font-display text-lg text-[#1B98E0]">{job.salary}</p>
                  <button className="bg-[#1B98E0] hover:bg-[#247BA0] text-[#13293D] px-6 py-2.5 font-bold text-sm uppercase transition-colors shadow-[3px_3px_0px_0px_rgba(36,123,160,1)]">
                    Candidatar-me
                  </button>
                </div>
              </div>
          ...
          {/* RECRUITER CTA */}
          <section className="bg-white/5 border-2 border-white/10 p-6 sm:p-12 md:p-16 text-center shadow-[12px_12px_0px_0px_rgba(36,123,160,0.3)]">
            ...
            <p className="font-display text-xl sm:text-3xl text-[#247BA0] uppercase mb-8">Encontre o Talento Certo.</p>
            ...
            <Link href="/cadastro" className="inline-flex items-center gap-3 bg-[#1B98E0] hover:bg-[#247BA0] text-[#13293D] px-6 sm:px-12 py-3 sm:py-5 font-display text-lg sm:text-2xl uppercase transition-colors shadow-[6px_6px_0px_0px_rgba(36,123,160,1)]">
      ...
      <footer className="bg-[#13293D] py-12 px-6 border-t border-white/10">
        <div className="max-w-[1400px] mx-auto">
          <div className="grid md:grid-cols-4 gap-12 mb-12">
            <div>
              <div className="font-display text-4xl text-[#1B98E0] mb-4 font-black italic tracking-tighter">TALENTOS</div>
